...
fitt <- function(datax,x){
  ...
  fP <- fit(dataP,arrest="tau",free=free,hmax=0.01,solution=T,
            odes=plsma,state=s,parms=p,ymax=0.05,
  ...
  s <- c(l1=0); p <- c(tau=49,10^fP$par,d1=0.1)
  free <- names(p)[-c(2,3,4)]
  lower <- c(log10(p[1]),-5); upper <- c(p[1],5)
  p <- log10(p); upper <- log10(upper)
  
  fC41p <- fit(dataC4,arrest="tau",free=free,hmax=0.01,fun=func,
              odes=chain1,state=s,parms=p,ymax=0.05,timeplot=F,
              lower=lower,upper=upper,method="Pseudo",
              xlab="Time (in days)",ylab="Fraction labelled")
  ...
  s <- c(l1=0,l2=0); p <- c(tau=49,10^fP$par,d1=0.1,d2=0.1,alp2=0.1)
  free <- names(p)[-c(1,2,3,4)]
  lower <- -5; upper <- c(5,5,1); p <- log10(p); upper <- log10(upper)
  ...
  p <- c(tau=49,10^fP$par,d0=0.1,d1=0.1,alp1=0.1,d2=0.1,alp2=0.1)
  s <- c(l0=0,l1=0,l2=0); free <- names(p)[-c(1,2,3,4)]
  lower <- -5; upper <- c(5,5,1,5,1)
  p <- log10(p); upper <- log10(upper)
...
ggData <- data.frame(); ggSim <- data.frame()
modnames <- c('(c) Single population',
              '(b) Two populations','(a) Three populations')
datai <- data[data$ID=='DW02',c(1,2,3)]; ests <- fitt(datai,'DW02')
print('Plasma'); print(ests[1:3])
print('a'); print(ests[9:13])
print('b'); print(ests[6:8]); print('c'); print(ests[5])

par(mar=c(2.6,2.6,1.6,0.2),mgp=c(1.5,0.5,0),mfrow=c(1,1))
print(ggplot(ggData,aes(x=time,y=l,color=sett))+geom_point()+
        facet_wrap(~modd)+theme_test()+xlim(0,130)+ylim(0,0.05)+
        geom_line(data=ggSim,aes(x=time,y=l,color=sett,group=sett))+
        geom_vline(aes(xintercept=49),size=0.4,linetype='dashed')+
        labs(y='Fraction labelled',x='Time (in days)')+
        theme(plot.title=element_text(hjust=0.5),legend.text.align=0.3,
              strip.background=element_blank())+
        scale_color_manual(name='Subsets',values=c("green","blue","red"),
                           labels=c("Precursors","CD57-","CD57+"),
                           guide=guide_legend(override.aes=list(shape=NA))))

ggsave('Figure7.pdf',width=8,height=5)
ggsave('Figure7.svg',width=8,height=5)
ggsave('Figure7.png',width=8,height=5)
