...
iter <- function(p){
  ...
  points(data1$time,data1$l2,pch=16)
  
  return(c(p,fit1$par,fit1$ssr))
}

tau <- 1; tstp <- 0.1; k <- 1; tmax <- 5
...
pdf_combine(mixedsort(list.files(pattern='.pdf')),output='FigureSX2_Fits.pdf')
...
#save.image('FigureS5.RData')
#ggsave('FigureS5.pdf',width=8,height=5)
#ggsave('FigureS5.svg',width=8,height=5)
#ggsave('FigureS5.png',width=8,height=5)
