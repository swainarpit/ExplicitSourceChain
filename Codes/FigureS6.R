...
fn <- function(p){
  ...
  data1[which(data1$time==tau),] <- NA; data1 <- na.omit(data1)
  data0 <- na.omit(data.frame(time=data1$time,l=data1$l2))
  temp <- as.vector(sapply(data1$l2, function(i) rnorm(2,i,i*0.2)))
  data <- data.frame(time=rep(data1$time,each=2),l=temp)
  ...
  tempD0 <- data.frame(data0$time,data0$l,x)
  names(tempD0) <- c('time','var','panel')
  dataD0 <<- rbind(dataD0,tempD0)
...
#load('FigureSX2.RData')
set.seed(1)
d1_v <- c(); p2_v <- c(); d2_v <- c(); ps_v <- c(); ds_v <- c(); ssr_v <- c()
dataD <- data.frame(); dataD0 <- data.frame(); dataF <- data.frame()
...
dataD0$panel <- factor(dataD0$panel,labels=panells)
...
print(ggplot(dataD,aes(x=time,y=var))+geom_point()+
        geom_point(data=dataD0,col=2,shape=1,size=2)+
...
#ggsave('FigureS6.pdf',width=8,height=5)
#ggsave('FigureS6.svg',width=8,height=5)
#ggsave('FigureS6.png',width=8,height=5)
