...
fit <- function(datas=data, state=s, parms=p, odes=model, free=NULL, who=NULL, differ=NULL, fixed=NULL, tmin=0, tmax=NULL, ymin=NULL, ymax=NULL, log="", xlab="Time", ylab="Density", bootstrap=0, data_weights=NULL, show=NULL, fun=NULL, costfun=cost, initial=FALSE, add=FALSE, timeplot=TRUE, legend=TRUE, main=NULL, sub=NULL, pchMap=NULL, npop = NULL, ...) {
  dots <- list(...)
  if (!is.null(dots)) {
  ...
  f <- do.call('modFit',c(list(f=costfun,p=guess,datas=datas,odes=odes,state=state,parms=parms,free=free,differ=differ,fixed=fixed,fun=fun,initial=initial,isVar=isVar,npop=npop),dots_fit))
  found <- f$par
  ...
  if (bootstrap == 0) return(f)
  else if (!is.null(data_weights)) { # fractional-random-weight bootstrapping starts here
    imat <- pbmclapply(seq(bootstrap), function(i) {
      ...
      ifit <- do.call('modFit',c(list(f=costfun,p=found,datas=samples,odes=odes,state=state,parms=parms,free=free,differ=differ,fixed=fixed,fun=fun,initial=initial,isVar=isVar,data_weights=data_weights,npop=npop),dots_fit))
    ...
    imat <- pbmclapply(seq(bootstrap), function(i) {
      ...
      ifit <- do.call('modFit',c(list(f=costfun,p=found,datas=samples,odes=odes,state=state,parms=parms,free=free,differ=differ,fixed=fixed,fun=fun,initial=initial,isVar=isVar,npop=npop),dots_fit))
      ifit$par
    },mc.cores=cores,ignore.interactive=T)
  }
...
cost <- function(datas, odes, state, parms, guess, free, differ, fixed, fun, initial, isVar, data_weights=NULL, ...) {
  dots <- list(...)
  if (!is.null(dots)) {
    dots_run <- dots[names(dots) %in% args_run]
