...
library(pbmcapply)   # to parallelize the bootstraps and have a progress bar
...
run <- function(tmax=100, tstep=1, state=s, parms=p, odes=model, ymin=0, ymax=NULL, log="", x=1, y=2, xlab="Time", ylab="Density", tmin=0, draw=lines, times=NULL, show=NULL, arrest=NULL, events=NULL, after=NULL, tweak=NULL, timeplot=TRUE, traject=FALSE, table=FALSE, add=FALSE, legend=TRUE, solution=FALSE, delay=FALSE, lwd=2, col="black", pch=20, mxhist=1e+4, ...) {   
  ...
  #if(!is.null(arrest)) {
  #  addarrest <- grep('\\+',arrest,value=T)
  #  if(length(addarrest) > 0){
  #    subaddarrest <- strsplit(addarrest,'\\+')
  #    addtimes <- sapply(seq(length(subaddarrest)), function(i) {
  #      ij <- subaddarrest[[i]]
  #      sum(sapply(seq(length(ij)), function(ik) {
  #        ifelse(ij[ik] %in% names(parms),as.numeric(parms[ij[ik]]),get(ij[ik]))
  #      }))
  #    })
  #    arrest <- setdiff(arrest,addarrest)
  #  } else addtimes <- NULL
  #  numindex <- suppressWarnings(!is.na(as.numeric(arrest)))
  #  times <- as.numeric(c(arrest[numindex],parms[arrest[!numindex]],addtimes,times))
  #  times <- unique(round(sort(times),8))
  #}
  if (solution) {                              # Run in one go
    ...
    if (is.null(after)){                       # Run in one go 
      ...
      else nsol <- as.data.frame(do.call('dede',c(list(times=times,func=odes,y=state,parms=parms,events=events,control=list(mxhist=mxhist)),dots_run)))
...
fit <- function(datas=data, state=s, parms=p, odes=model, free=NULL, who=NULL, differ=NULL, fixed=NULL, tmin=0, tmax=NULL, ymin=NULL, ymax=NULL, log="", xlab="Time", ylab="Density", bootstrap=0, data_weights=NULL, boot_type=NULL, cost_type=NULL, show=NULL, fun=NULL, costfun=cost, initial=FALSE, add=FALSE, timeplot=TRUE, legend=TRUE, main=NULL, sub=NULL, pchMap=NULL, npop = NULL, ...) {
  dots <- list(...)
  if (!is.null(dots)) {
  ...
  f <- do.call('modFit',c(list(f=costfun,p=guess,datas=datas,odes=odes,state=state,parms=parms,free=free,differ=differ,fixed=fixed,fun=fun,initial=initial,isVar=isVar,npop=npop,cost_type=cost_type),dots_fit))
  found <- f$par
  ...
  cores <- ifelse(.Platform$OS.type=="windows",1,detectCores()-1)
  if (bootstrap == 0) return(f)
  else if (!is.null(boot_type)){
    if(boot_type=='residuals') {
      imat <- pbmclapply(seq(bootstrap), function(i) {
        samples <- lapply(seq(nsets),function(j){
          for(k in seq(2,dim(datas[[j]])[2])){
            nam_col <- names(datas[[j]])[k]
            nonNA_rows <- which(!is.na(datas[[j]][[nam_col]]))
            res_sample <- sample(unname(f$residuals[which(names(f$residuals)==nam_col)]),replace=T)
            datas[[j]][nonNA_rows,k] <- datas[[j]][nonNA_rows,k] + res_sample
          }
          datas[[j]]
        })
        ifit <- do.call('modFit',c(list(f=costfun,p=found,datas=samples,odes=odes,state=state,parms=parms,free=free,differ=differ,fixed=fixed,fun=fun,initial=initial,isVar=isVar,npop=npop,cost_type=cost_type),dots_fit))
        return(c(ifit$par,ssr=ifit$ssr))
      },mc.cores=cores,ignore.interactive=T)
    }
    else print('Do not use the parameter boot_type if you want to bootstrap data pairs; the parameter boot_type can only take \'residuals\' as its value.')
  } else {
    if (!is.null(data_weights)) { # fractional-random-weight bootstrapping starts here
      imat <- pbmclapply(seq(bootstrap), function(i) {
        samples <- lapply(seq(nsets),function(j){
          x <- cross2long(datas[[j]],x=time,error=TRUE,na.rm=FALSE); names(x)[names(x) == "x"] <- "time"
          if(data_weights == "random") x$err <- 1/(diff(sort(c(0,runif(nrow(x)-1,0,1),1)))*nrow(x)) # random weights are generated here
          else x$err <- data_weights #data_weights also can be defined by user (must be in long format)
          x
        })
        ifit <- do.call('modFit',c(list(f=costfun,p=found,datas=samples,odes=odes,state=state,parms=parms,free=free,differ=differ,fixed=fixed,fun=fun,initial=initial,isVar=isVar,data_weights=data_weights,npop=npop,cost_type=cost_type),dots_fit))
        return(c(ifit$par,ssr=ifit$ssr))
      },mc.cores=cores,ignore.interactive=T)
    } else {
      imat <- pbmclapply(seq(bootstrap), function(i) {
  ...
  imat <- simplify2array(imat)
  if (length(found) == 1) imat <- matrix(imat,nrow=1,dimnames=list(names(found[1]),NULL))
  print(apply(imat, 1, function(i)c(mean=mean(i),sd=sd(i),median=median(i),quantile(i,c(.025, .975)))))
  f$bootstrap <- t(imat)
  return(f)
}

cost <- function(datas, odes, state, parms, guess, free, differ, fixed, fun, initial, isVar, data_weights=NULL, cost_type=NULL, ...) {
  dots <- list(...)
  if (!is.null(dots)) {
    dots_run <- dots[names(dots) %in% args_run]
  ...
  for (iset in seq(nsets)) {
    ...
    times <- sort(unique(data[,which(colnames(data) == "time")])) #use column time
    ...
    if (!is.null(fun) & is.null(data_weights)) {
    ...
    if(!is.null(data_weights)) totcost <- do.call('modCost',c(list(model=nsol,obs=data,cost=totcost,err="err",y="y"),dots_fit))
    else {
  ...
  if(!is.null(cost_type)){
    if(cost_type=='MLE'){
      x_mle <- totcost$var[,c(3,6)]
      mle_cost <- -0.5*log(prod(x_mle$SSR**x_mle$N))-sum(x_mle$N) #log likelihood
      totcost$model <- -mle_cost #Minimize the negative of log likelihood
    }
  }
  return (totcost)
}
