...
iter <- function(p){
  ...
  data <- data.frame(time=rep(data1$time,each=2),l=temp) #Save the required data
...
pdf_combine(mixedsort(list.files(pattern='.pdf')),output='FigH_Fits.pdf') #Combine all the pdfs
...
# save.image('FigH.RData')
# ggsave('FigH.pdf',width=8,height=5)
# ggsave('FigH.svg',width=8,height=5)
# ggsave('FigH.png',width=8,height=5)
