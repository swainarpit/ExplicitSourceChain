...
par(mar=c(2.6,2.6,1.6,0.2),mgp=c(1.5,0.5,0),mfrow=c(1,1))
ggData_b <- ggData[which(ggData$modd=='(b) Two populations'),1:3]
ggSim_b <- ggSim[which(ggSim$modd=='(b) Two populations'),1:3]

#Make the ggplot
print(ggplot(ggData_b,aes(x=time,y=l,color=sett))+geom_point()+
        theme_test()+xlim(0,130)+ylim(0,0.05)+
        geom_line(data=ggSim_b,aes(x=time,y=l,color=sett,group=sett,linetype=sett))+
        geom_vline(aes(xintercept=49),size=0.4,linetype='dashed')+
        labs(title='Two populations',y='Deuterium enrichment (APE)',x='Time (in days)')+
        theme(plot.title=element_text(hjust=0.5),
              legend.text.align=0.3,strip.background=element_blank())+
        scale_linetype_manual(name='Subsets',values=c(4,1),
                              labels=c("CD57-","CD57+"))+
        scale_color_manual(name='Subsets',values=c("darkred","dodgerblue"),
                           labels=c("CD57-","CD57+"),
                           guide=guide_legend(override.aes=list(shape=NA))))

# save.image('FigL.RData')
# ggsave('FigL.pdf',width=8,height=5)
# ggsave('FigL.svg',width=8,height=5)
# ggsave('FigL.png',width=8,height=5)
